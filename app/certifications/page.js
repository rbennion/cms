...
import { Award, CheckCircle, Clock, XCircle, AlertCircle, Upload } from 'lucide-react'
...
export default function CertificationsPage() {
  ...
  const fetchCertifications = async () => {
    ...
    try {
      ...
      if (filter.status) params.append('background_check_status', filter.status)
      ...
      toast({ title: 'Error', description: 'Failed to fetch certifications', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const endpoint = uploadType === 'application' ? 'application' : 'training'
      const res = await fetch(`/api/certifications/${uploadCert.id}/${endpoint}`, {
        method: 'POST',
        body: formData,
      })
  ...
  return (
    <div className="flex flex-col">
      <Header title="Certifications" description="Manage FC certification status" />
      ...
      <div className="p-6">
            ...
            return (
              <Card key={status}>
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                  <CardTitle className="text-sm font-medium capitalize">{config.label}</CardTitle>
        ...
        <div className="mb-6 flex gap-4">
          ...
          <Select
            ...
            onValueChange={(value) => setFilter(prev => ({ ...prev, training: value }))}
          >
                    ...
                    <TableRow key={cert.id}>
                      ...
                      <TableCell>
                        <div className="text-sm">
                          ...
                          {cert.phone && <div className="text-muted-foreground">{cert.phone}</div>}
                        </div>
                      </TableCell>
                      <TableCell>
                        <Badge
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Badge variant={cert.application_received ? 'success' : 'secondary'}>
                            {cert.application_received ? 'Received' : 'Not Received'}
                          </Badge>
                          <Button
                            size="sm"
                            variant="ghost"
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Badge
                            variant={cert.training_complete ? 'success' : 'secondary'}
                            className="cursor-pointer"
                            onClick={() => handleToggleTraining(cert.id, cert.training_complete)}
                          >
      ...
      {/* Upload Dialog */}
      <Dialog open={!!uploadCert && !!uploadType} onOpenChange={() => { setUploadCert(null); setUploadType(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Upload {uploadType === 'application' ? 'Application' : 'Training'} Document
            </DialogTitle>
          </DialogHeader>
          <div className="py-4">
