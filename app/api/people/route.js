...
export async function GET(request) {
  try {
    ...
    let query = `
      SELECT DISTINCT p.*,
        GROUP_CONCAT(DISTINCT pt.name) as types
      FROM people p
      LEFT JOIN person_type_assignments pta ON p.id = pta.person_id
      LEFT JOIN person_types pt ON pta.type_id = pt.id
      LEFT JOIN person_schools ps ON p.id = ps.person_id
      WHERE 1=1
    `
    const params = []
  ...
  try {
    ...
    const result = await run(
      `INSERT INTO people (first_name, middle_name, last_name, email, phone, title, address, city, state, zip, is_donor, is_fc_certified, is_board_member, children)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [first_name, middle_name || null, last_name, email || null, phone || null, title || null, address || null, city || null, state || null, zip || null, is_donor ? 1 : 0, is_fc_certified ? 1 : 0, is_board_member ? 1 : 0, children || null]
    )
