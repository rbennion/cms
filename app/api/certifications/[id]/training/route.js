...
export async function POST(request, { params }) {
  try {
    ...
    const oldPath = certification.qpr_training_attachment_path
    if (oldPath && oldPath !== blob.pathname && !oldPath.startsWith('http')) {
      try {
        await del(oldPath)
      } catch (cleanupError) {
        console.error('Failed to delete replaced training document:', cleanupError)
      }
    }

    return NextResponse.json({ qpr_training_attachment_path: blob.pathname })
  } catch (error) {
