...
async function seedDatabase() {
  ...
  for (const person of people) {
    await sql`INSERT INTO people (first_name, middle_name, last_name, email, phone, title, stage_id)
      VALUES (${person.first_name}, ${person.middle_name}, ${person.last_name}, ${person.email}, ${person.phone}, ${person.title}, ${person.stage_id})`;
  }
  ...
  await sql`INSERT INTO certifications (person_id, background_check_status, application_received, qpr_gatekeeper_training) VALUES (1, 'approved', true, true)`;
  await sql`INSERT INTO certifications (person_id, background_check_status, application_received, qpr_gatekeeper_training) VALUES (2, 'approved', true, true)`;
  await sql`INSERT INTO certifications (person_id, background_check_status, application_received, qpr_gatekeeper_training) VALUES (4, 'pending', true, false)`;
...
seedDatabase().catch(console.error).finally(() => pool.end());
