...
async function initDatabase() {
  ...
  // Create indexes
  await db.execute('CREATE INDEX IF NOT EXISTS idx_people_email ON people(email)')
  await db.execute('CREATE INDEX IF NOT EXISTS idx_people_last_name ON people(last_name)')
  await db.execute('CREATE INDEX IF NOT EXISTS idx_companies_name ON companies(name)')
  await db.execute('CREATE INDEX IF NOT EXISTS idx_donations_date ON donations(date)')
  await db.execute('CREATE INDEX IF NOT EXISTS idx_donations_person ON donations(person_id)')
  await db.execute('CREATE INDEX IF NOT EXISTS idx_donations_company ON donations(company_id)')
  await db.execute('CREATE INDEX IF NOT EXISTS idx_notes_entity ON notes(entity_type, entity_id)')

  // Seed default person types
  await db.execute("INSERT OR IGNORE INTO person_types (name) VALUES ('Lead')")
  await db.execute("INSERT OR IGNORE INTO person_types (name) VALUES ('Interested')")
